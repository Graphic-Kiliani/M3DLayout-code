...
class GaussianDiffusion:
    def __init__(self, config, betas, loss_type, model_mean_type, model_var_type, loss_separate, loss_iou, train_stats_file):
        ...
        # NEW: Add timestep-staged training configuration
        self.enable_timestep_staged_training = config.get("enable_timestep_staged_training", False)
        self.staged_training_large_weight_early = config.get("staged_training_large_weight_early", 1.0)
        self.staged_training_small_weight_early = config.get("staged_training_small_weight_early", 0.1)
        self.staged_training_large_weight_later = config.get("staged_training_large_weight_later", 0.2)
        self.staged_training_small_weight_later = config.get("staged_training_small_weight_later", 1.0)
        
        if self.enable_timestep_staged_training:
            print(f"Timestep-staged training enabled:")
            print(f"  Early stage (large objects focus): large_weight={self.staged_training_large_weight_early}, small_weight={self.staged_training_small_weight_early}")
            print(f"  Later stage (small objects focus): large_weight={self.staged_training_large_weight_later}, small_weight={self.staged_training_small_weight_later}")
        
        if self.loss_iou:
            # train_stats_file can be a file path (str) or an inline dict
            if isinstance(train_stats_file, dict):
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, condition=None, condition_cross=None, object_size_masks=None):
        """
        Training loss calculation with optional timestep-staged training for large/small objects
        """
        ...
        if self.loss_type == 'mse':
            ...
            if self.room_arrange_condition:
                ...
                # NEW: Apply timestep-staged training for large/small objects
                if object_size_masks is not None and hasattr(self, 'enable_timestep_staged_training') and self.enable_timestep_staged_training:
                    # object_size_masks: [B, N] - 1 for large objects, 0 for small objects
                    large_object_mask = object_size_masks  # [B, N]
                    small_object_mask = 1 - large_object_mask  # [B, N]
                    
                    # Determine stage based on timestep
                    # Early timesteps (t > num_timesteps/2): focus on large objects
                    # Later timesteps (t <= num_timesteps/2): focus on small objects
                    timestep_threshold = self.num_timesteps // 2
                    is_early_stage = t > timestep_threshold  # [B]
                    
                    # Compute separate losses for large and small objects
                    # For loss calculation, we need per-object losses instead of per-batch losses
                    if self.loss_separate:
                        # Recompute per-object losses without mean reduction across objects
                        loss_bbox_per_obj = ((target[:, :, 0:self.bbox_dim] - denoise_out[:, :, 0:self.bbox_dim])**2).mean(dim=-1)  # [B, N]
                        loss_class_per_obj = ((target[:, :, self.bbox_dim:self.bbox_dim+self.class_dim] - denoise_out[:, :, self.bbox_dim:self.bbox_dim+self.class_dim])**2).mean(dim=-1)  # [B, N]
                        losses_per_obj = loss_bbox_per_obj + loss_class_per_obj  # [B, N]
                        
                        if self.objectness_dim > 0:
                            loss_object_per_obj = ((target[:, :, self.bbox_dim+self.class_dim:self.bbox_dim+self.class_dim+self.objectness_dim] - denoise_out[:, :, self.bbox_dim+self.class_dim:self.bbox_dim+self.class_dim+self.objectness_dim])**2).mean(dim=-1)
                            losses_per_obj += loss_object_per_obj
                        if self.objfeat_dim > 0:
                            loss_objfeat_per_obj = ((target[:, :, self.bbox_dim+self.class_dim+self.objectness_dim:] - denoise_out[:, :, self.bbox_dim+self.class_dim+self.objectness_dim:])**2).mean(dim=-1)
                            losses_per_obj += loss_objfeat_per_obj
                    else:
                        losses_per_obj = ((target - denoise_out)**2).mean(dim=-1)  # [B, N]
                    
                    # Apply stage-specific weighting using configured weights
                    # Create weight masks for each batch item
                    weight_mask = torch.zeros_like(losses_per_obj)  # [B, N]
                    for b in range(B):
                        if is_early_stage[b]:
                            # Early stage: emphasize large objects
                            weight_mask[b] = large_object_mask[b] * self.staged_training_large_weight_early + small_object_mask[b] * self.staged_training_small_weight_early
                        else:
                            # Later stage: emphasize small objects
                            weight_mask[b] = large_object_mask[b] * self.staged_training_large_weight_later + small_object_mask[b] * self.staged_training_small_weight_later
                    
                    # Apply the weighting and compute batch-level losses
                    weighted_losses_per_obj = losses_per_obj * weight_mask  # [B, N]
                    losses = weighted_losses_per_obj.mean(dim=1)  # [B]
...
class DiffusionPoint(nn.Module):
    ...
    def get_loss_iter(self, data, noises=None, condition=None, condition_cross=None, source_weights=None, object_size_masks=None):
        ...
        losses, loss_dict = self.diffusion.p_losses(
            ...
            condition=condition, condition_cross=condition_cross, object_size_masks=object_size_masks)
        assert losses.shape == t.shape == torch.Size([B])
        
        # Apply source weights if provided
        if source_weights is not None:
            weighted_losses = losses * source_weights
            return weighted_losses.mean(), loss_dict
        else:
