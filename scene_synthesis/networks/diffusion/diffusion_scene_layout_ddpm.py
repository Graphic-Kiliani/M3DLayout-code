...
class DiffusionSceneLayout_DDPM(Module):
    ...
    def __init__(self, n_classes, feature_extractor, config):
        ...
        # Two-stage training: use large objects as partial condition
        self.two_stage_small_from_large = config.get("two_stage_small_from_large", False)
        
        if self.room_partial_condition or self.two_stage_small_from_large:
    ...
    def get_loss(self, sample_params):
        ...
        if self.config["point_dim"] == self.bbox_dim+self.class_dim:
        ...
        # concat room_partial condition or two-stage large objects condition
        ...
        elif self.two_stage_small_from_large and "large_partial_input" in sample_params:
            # Use large objects as partial condition
            large_partial_input = sample_params["large_partial_input"]  # (batch_size, max_large_objects, feature_dim)
            num_large_objects = sample_params["num_large_objects"]  # (batch_size,)
            
            # Create padded partial input for the full sequence
            partial_input = torch.zeros_like(room_layout_target).to(device)
            
            for b in range(batch_size):
                n_large = int(num_large_objects[b].item())
                if n_large > 0:
                    # Place large objects at the beginning of the sequence
                    partial_input[b, :n_large, :] = large_partial_input[b, :n_large, :]
            
            partial_condition_f = self.fc_partial_condition(partial_input)
            if condition is None:
                condition = partial_condition_f
            else:
                condition = torch.cat([condition, partial_condition_f], dim=-1).contiguous()
        ...
        source_weights = sample_params.get('source_weights', None)
        
        # NEW: Add timestep-staged training for large/small objects
        object_size_masks = sample_params.get('object_size_masks', None)
        loss, loss_dict = self.diffusion.get_loss_iter(
            room_layout_target, 
            condition=condition, 
            condition_cross=condition_cross, 
            source_weights=source_weights,
            object_size_masks=object_size_masks
        )
    ...
    @torch.no_grad()
    def generate_layout(self, room_mask, num_points, point_dim, batch_size=1, text=None, partial_boxes=None, ret_traj=False, ddim=False, clip_denoised=False, batch_seeds=None, device="cpu", keep_empty=False):
    ...
    def delete_empty_from_network_samples(self, samples, device="cpu", keep_empty=False):
        ...
        boxes = {
            ...
            "class_labels": torch.zeros(1, 0, self.class_dim-1, device=device),  # class_dim-1 matches loop
...
def train_on_batch(model, optimizer, sample_params, config):
    ...
    loss, loss_dict = model_module.get_loss(sample_params)
