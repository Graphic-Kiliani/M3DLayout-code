...
def build_network(
    ...
    if weight_file is not None:
        ...
        # Support both plain state_dicts and full checkpoints that bundle
        # optimizer state alongside the model (keys: 'state', 'param_groups').
        if isinstance(ckpt, dict):
            if "model" in ckpt:
                state_dict = ckpt["model"]
            elif "state_dict" in ckpt:
                state_dict = ckpt["state_dict"]
            elif "state" in ckpt and "param_groups" in ckpt:
                # This is an optimizer state_dict, not a model – skip it gracefully
                print("WARNING: weight file appears to be an optimizer checkpoint; skipping.")
                state_dict = None
            else:
                state_dict = ckpt
        else:
        ...
        if state_dict is not None:
            ...
            if missing:
                print("WARNING: missing keys in state_dict ({} keys)".format(len(missing)))
            if unexpected:
                print("WARNING: unexpected keys in state_dict ({} keys, ignored)".format(len(unexpected)))
    network.to(device)
